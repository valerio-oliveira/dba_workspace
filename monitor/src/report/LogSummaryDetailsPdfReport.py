...
class LogSummaryDetailsPdfReport:
    def get(hosts_summary, summary_details, txt):
        pdf = DefaultPdfReport(title="Log Detalhado de Erros")
        # it is necessary to set font before access pdf.get_string_width
        ...
        for host_line in hosts_summary:
            ...
            for lin in lin_list:
                ...
                for summary_detail in summary_details:
                    host_detail = summary_detail["host_name"]
                    if summary_detail["host_name"] == host_name.replace("-", "_"):
                        error_seq = summary_detail["error_seq"]
                        if error_seq == lin_seq:
                            pdf.set_text_color(160, 50, 50)  # red
                            for err in summary_detail["err_list"]:
                                pdf.set_x(15)
                                pdf.multi_cell(w=0, h=5, txt=err,
                                               new_x="LMARGIN", new_y="NEXT")
                                pdf.set_text_color(50, 50, 160)  # blue
